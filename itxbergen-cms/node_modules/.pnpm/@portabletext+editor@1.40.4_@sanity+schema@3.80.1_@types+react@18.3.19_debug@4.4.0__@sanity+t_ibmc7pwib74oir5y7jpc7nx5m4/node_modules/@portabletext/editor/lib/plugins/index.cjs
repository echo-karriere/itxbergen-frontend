  ...
  return behavior_core.defineBehavior({
    ...
    guard: ({
      ...
      if (suffixOffsets.focus.offset - suffixOffsets.anchor.offset > 1) {
        const previousInlineObject = selector_isAtTheStartOfBlock.getPreviousInlineObject(snapshot), previousInlineObjectOffset = previousInlineObject ? util_selectionPointToBlockOffset.childSelectionPointToBlockOffset({
          value: snapshot.context.value,
          selectionPoint: {
...
const oneLineBehaviors = [
  ...
  behavior_core.defineBehavior({
    ...
    guard: ({
      snapshot,
      event
    }) => !(!selector_isAtTheStartOfBlock.getFocusTextBlock(snapshot) || !util_mergeTextBlocks.isTextBlock(snapshot.context, event.block)),
    actions: [({
      event
    }) => [{
      type: "insert.block",

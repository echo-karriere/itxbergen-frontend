...
const browserCompatibleSanityPackageSpecifiers = ["sanity", "sanity/_createContext", "sanity/_singletons", "sanity/desk", "sanity/presentation", "sanity/router", "sanity/structure", "sanity/package.json"], conditions = ["import", "browser", "default"];
...
function getDocumentComponent(studioRootPath, isApp) {
  ...
  const {
    ...
    DefaultDocument
  } = require("sanity"), Document = isApp ? BasicDocument : DefaultDocument;
...
async function getViteConfig(options) {
  ...
  const customFaviconsPath = path__default.default.join(cwd, "static"), defaultFaviconsPath = path__default.default.join(path__default.default.dirname(sanityPkgPath), "static", "favicons"), staticPath = `${basePath}static`, {
    default: viteReact
  } = await import("@vitejs/plugin-react"), viteConfig = {
...
function getEntryModule(options) {
  ...
  return isApp ? appEntryModule.replace(/%APP_LOCATION%/, JSON.stringify(appLocation || "./src/App")) : (relativeConfigLocation ? entryModule : noConfigEntryModule).replace(/%STUDIO_REACT_STRICT_MODE%/, JSON.stringify(!!reactStrictMode)).replace(/%STUDIO_CONFIG_LOCATION%/, JSON.stringify(relativeConfigLocation)).replace(/%STUDIO_BASE_PATH%/, JSON.stringify(basePath || "/"));
}
...
async function writeSanityRuntime({
  ...
  const relativeAppLocation = cwd ? path__default.default.resolve(cwd, appLocation || "./src/App") : appLocation, appJsContent = getEntryModule({
    reactStrictMode,
    relativeConfigLocation,
    basePath,
